...
impl CertificationAuthority {
    // Taille maximale du compte (calculée avec InitSpace)
    pub const MAX_SIZE: usize = 8 + 32 + 4 + (32 * 50) + 8 * 5 + 32 + 1;
    ...
    pub fn increment_counter(&mut self, cert_type: &crate::state::CertificationType) -> Result<()> {
        self.total_issued = self.total_issued.checked_add(1).ok_or(ProgramError::ArithmeticOverflow)?;
        match cert_type {
            crate::state::CertificationType::Standard => {
                self.standard_count = self.standard_count.checked_add(1).ok_or(ProgramError::ArithmeticOverflow)?;
            },
            crate::state::CertificationType::Premium => {
                self.premium_count = self.premium_count.checked_add(1).ok_or(ProgramError::ArithmeticOverflow)?;
            },
            crate::state::CertificationType::Luxury => {
                self.luxury_count = self.luxury_count.checked_add(1).ok_or(ProgramError::ArithmeticOverflow)?;
            },
            crate::state::CertificationType::Exceptional => {
                self.exceptional_count = self.exceptional_count.checked_add(1).ok_or(ProgramError::ArithmeticOverflow)?;
            },
        }
        Ok(())
    }
